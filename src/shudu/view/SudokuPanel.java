...
public class SudokuPanel extends JPanel implements Observer {
    ...
    private Color errorco=new Color(212,212,212);
    private Color checo=new Color(144,196,254);
    private Color rightco=new Color(186,255,145);
        ...
        for (int y = 0; y < 9; y++) {
            for (int x = 0; x < 9; x++) {
                ...
                if (fields[y][x].getForeground().equals(Color.BLUE))
                    fields[y][x].setBackground(game.isCheckValid(x, y) ? rightco : errorco);
            }
        }
    }
